...
return {
	{
		label = "Host",
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
...
A single argument of \"none\" causes no CNAMEs to be considered for canonicalization. This is the default behaviour.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies which algorithms are allowed for signing of certificates by certificate authorities (CAs). The default is: ssh-ed25519,ecdsa-sha2-nistp256, ecdsa-sha2-nistp384,ecdsa-sha2-nistp521, sk-ssh-ed25519@openssh.com, sk-ecdsa-sha2-nistp256@openssh.com, rsa-sha2-512,rsa-sha2-256
...
ssh(1) will not accept host certificates signed using algorithms other than those specified.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies a file from which the user's certificate is read. A corresponding private key must be provided separately in order to use this certificate either from an **IdentityFile** directive or -i flag to [ssh(1)], via [ssh-agent(1)], or via a **PKCS11Provider** or **SecurityKeyProvider**.
...
It is possible to have multiple certificate files specified in configuration files; these certificates will be tried in sequence. Multiple CertificateFile directives will add to the list of certificates used for authentication.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies whether and how quickly [ssh(1)] should close inactive channels. Timeouts are specified as one or more “type=interval” pairs separated by whitespace, where the “type” must be the special keyword “global” or a channel type name from the list below, optionally containing wildcard characters.
...
Two additional options allow for opportunistic multiplexing: try to use a master connection but fall back to creating a new one if one does not already exist. These options are: auto and autoask. The latter requires confirmation like the ask option.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies that a TCP port on the local machine be forwarded over the secure channel, and the application protocol is then used to determine where to connect to from the remote machine.
...
If the ExitOnForwardFailure configuration option is set to “yes”, then a client started with the ForkAfterAuthentication configuration option being set to “yes” will wait for all remote port forwards to be successfully established before placing itself in the background. The argument to this keyword must be yes (same as the -f option) or no (the default).
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies whether the connection to the authentication agent (if any) will be forwarded to the remote machine. The argument may be **yes**, **no** (the default), an explicit path to an agent socket or the name of an environment variable (beginning with ‘$’) in which to find the path.
...
Agent forwarding should be enabled with caution. Users with the ability to bypass file permissions on the remote host (for the agent's Unix-domain socket) can access the local agent through the forwarded connection. An attacker cannot obtain key material from the agent, however they can perform operations on the keys that enable them to authenticate using the identities loaded into the agent.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies whether X11 connections will be automatically redirected over the secure channel and DISPLAY set. The argument must be **yes** or **no** (the default).
...
The list of available signature algorithms may also be obtained using \"ssh -Q HostKeyAlgorithms\".
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies the UNIX-domain socket used to communicate with the authentication agent.
...
IdentityFile may be used in conjunction with IdentitiesOnly to select which identities in an agent are offered during authentication. IdentityFile may also be used in conjunction with CertificateFile in order to provide any certificate also needed for authentication with the identity.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies the permitted KEX (Key Exchange) algorithms that will be used and their preference order. The selected algorithm will be the first algorithm in this list that the server also supports. Multiple algorithms must be comma-separated.
...
The list of supported key exchange algorithms may also be obtained using \"ssh -Q kex\".
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies a command to execute on the local machine after successfully connecting to the server. The command string extends to the end of the line, and is executed with the user's shell. Arguments to **LocalCommand** accept the tokens described in the [TOKENS] section.
...
Multiple forwardings may be specified, and additional forwardings can be given on the command line. Only the superuser can forward privileged ports. By default, the local port is bound in accordance with the GatewayPorts setting. However, an explicit bind_address may be used to bind the connection to a specific address. The bind_address of localhost indicates that the listening port be bound for local use only, while an empty address or ‘*’ indicates that the port should be available from all interfaces. Unix domain socket paths may use the tokens described in the TOKENS section and environment variables as described in the ENVIRONMENT VARIABLES section.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specify one or more overrides to LogLevel. An override consists of one or more pattern lists that matches the source file, function and line number to force detailed logging for. For example, an override pattern of: kex.c:*:1000,*:kex_exchange_identification():*,packet.c:*
...
The list of available MAC algorithms may also be obtained using \"ssh -Q mac\".
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies the destinations to which remote TCP port forwarding is permitted when **RemoteForward** is used as a SOCKS proxy. The forwarding specification must be one of the following forms:
...
Multiple forwards may be specified by separating them with whitespace. An argument of any can be used to remove all restrictions and permit any forwarding requests. An argument of none can be used to prohibit all forwarding requests. The wildcard ‘*’ can be used for host or port to allow all hosts or ports respectively. Otherwise, no pattern matching or address lookups are performed on supplied names.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies the command to use to connect to the server. The command string extends to the end of the line, and is executed using the user's shell ‘exec’ directive to avoid a lingering shell process.
...
If the bind_address is not specified, the default is to only bind to loopback addresses. If the bind_address is ‘*’ or an empty string, then the forwarding is requested to listen on all interfaces. Specifying a remote bind_address will only succeed if the server's GatewayPorts option is enabled (see sshd_config(5)).
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies a path to a library that will be used when loading any FIDO authenticator-hosted keys, overriding the default of using the built-in USB HID support.
...
The default value is 3. If, for example, ServerAliveInterval (see below) is set to 15 and ServerAliveCountMax is left at the default, if the server becomes unresponsive, ssh will disconnect after approximately 45 seconds.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Directly specify one or more environment variables and their contents to be sent to the server in the form “NAME=VALUE”. Similarly to **SendEnv**, with the exception of the TERM variable, the server must be prepared to accept the environment variable.
...
The “VALUE” may use the tokens described in the TOKENS section and environment variables as described in the ENVIRONMENT VARIABLES section.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Sets the octal file creation mode mask (umask) used when creating a Unix-domain socket file for local or remote port forwarding. This option is only used for port forwarding to a Unix-domain socket file.
...
If this flag is set to accept-new then ssh will automatically add new host keys to the user's known_hosts file, but will not permit connections to hosts with changed host keys. If this flag is set to no or off, ssh will automatically add new host keys to the user known hosts files and allow connections to hosts with changed hostkeys to proceed, subject to some restrictions. If this flag is set to ask (the default), new host keys will be added to the user known host files only after the user has confirmed that is what they really want to do, and ssh will refuse to connect to hosts whose host key has changed. The host keys of known hosts will be verified automatically in all cases.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies whether the system should send TCP keepalive messages to the other side. If they are sent, death of the connection or crash of one of the machines will be properly noticed. However, this means that connections will die if the route is down temporarily, and some people find it annoying.
...
To disable TCP keepalive messages, the value should be set to no. See also ServerAliveInterval for protocol-level keepalives.
	...
	},
	{
		...
		kind = require("blink.cmp.types").CompletionItemKind.Keyword,		documentation = {
			kind = "markdown",
			value = [[Specifies the [tun(4)] devices to open on the client (|local_tun|) and the server (|remote_tun|).
